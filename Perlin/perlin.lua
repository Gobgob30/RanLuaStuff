...
local function noise(x, y, z)
    y = y or 0
    z = z or 0
    local a, b, c = math.floor(x) % 256, math.floor(y) % 256, math.floor(z) % 256
...
--     local ab = noise(x, y, z)
--     local bc = noise(y, z, z)
--     local ac = noise(x, z, z)

--     local ba = noise(y, x, z)
--     local cb = noise(z, y, z)
--     local ca = noise(z, x, z)
...
local function perlin_2d(x, y, scale, octaves, persistance, lacunarity, seed)
    seed = seed or math.random(1, 9999999)
    x, y = x + seed, y + seed
    local value = 0
    local frequency = 1
    local amplitude = 1
    local max = 0
    for i = 1, octaves do
        value = value + noise(x * scale * frequency, y * scale * frequency) * amplitude
        max = max + amplitude
...
local function perlin_3d(x, y, z, scale, octaves, persistance, lacunarity, seed)
    seed = seed or math.random(1, 9999999)
    x, y, z = x + seed, y + seed, z + seed
    local value = 0
    local frequency = 1
    local amplitude = 1
    local max = 0
    for i = 1, octaves do
        value = value + noise(x * scale * frequency, y * scale * frequency, z * scale * frequency) * amplitude
        max = max + amplitude
        amplitude = amplitude * persistance
        frequency = frequency * lacunarity
        yield()
